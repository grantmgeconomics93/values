...
wave1select <- wave1 %>%
  ...
  select("religious denomination", "Self positioning in political scale", "age", "employment", "sex","cheat","country","educ","Socio-economic status of respondent")
...
wave4select <- wave4 %>%
  ...
  select("religious denomination", "Self positioning in political scale", "age", "employment", "sex","educ","cheat","Socio-economic status of respondent", "country")
...
wave2select <- wave2select %>%
  mutate(`Socio-economic status of respondent` = case_when(
    `Socio-economic status of respondent` == 1 ~ "Upper class",
    `Socio-economic status of respondent` == 2 ~ "Upper middle class",
    `Socio-economic status of respondent` == 3 ~ "Upper middle class",
...
wave3select <- wave3select %>%
  # Convert 'educ' to numeric; this assumes 'educ' is already a character or factor
  mutate(educ = as.numeric(educ)) %>%
  # Now filter out rows where 'educ' is not greater than 0
  filter(educ > 0)
