...
```{r}
# Fit a probit model
probit_model <- glm(cheat ~sex+country+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+ Year, data =final_df , family = binomial(link = "probit"))
```
```{r}
library(margins)
