...
PseudoR2(original ,c("McFaddenAdj","McKelveyZavoina"))
```
...
```{r}
 ...
 PseudoR2(raw,c("McFaddenAdj","McKelveyZavoina"))
```
...
probit_region  <-glm(cheat ~region+sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+Year, data =merged_df , family = binomial(link = "probit"))
 PseudoR2(probit_region,c("McFaddenAdj","McKelveyZavoina"))

```
```{r}
corruption   <- glm(cheat ~score+growth+incleval+sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+Year, data =merged_df , family = binomial(link = "probit"))
 PseudoR2(corruption,c("McFaddenAdj","McKelveyZavoina"))
```
...
bothcorandregion <- glm(cheat ~region+growth+incleval+score+sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+Year, data =merged_df , family = binomial(link = "probit"))
 PseudoR2(bothcorandregion,c("McFaddenAdj","McKelveyZavoina"))

```
```{r}
marginal_effects <- margins(bothcorandregion,data = merged_df)
marginal_summary=summary(marginal_effects)
```

```{r}
bothcorregeffects <- data.frame(
  Term = marginal_summary$factor,
  AME = marginal_summary$AME,
  SE = marginal_summary$SE,
  z = marginal_summary$z,
...
cor(as.numeric(merged_df$incleval),as.numeric(merged_df$cheat))
