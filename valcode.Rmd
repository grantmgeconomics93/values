...
wave1select <- wave1 %>%
  ...
  select("religious denomination", "Self positioning in political scale", "employment", "sex","cheat","country","educ","Socio-economic status of respondent","age")
...
wave2select <- wave2 %>%
  ...
  select("religious denomination", "Self positioning in political scale", "age", "employment", "sex","cheat","educ","Socio-economic status of respondent","country")
...
wave4select <- wave4 %>%
  ...
  select("religious denomination", "Self positioning in political scale", "employment", "sex","educ","cheat","Socio-economic status of respondent", "country","age")
...
wave3select <- wave3select %>%
  mutate(`Socio-economic status of respondent` = case_when(
    `Socio-economic status of respondent` == 1 ~ "Upper class",
    `Socio-economic status of respondent` == 2 ~ "Upper class",
    `Socio-economic status of respondent` == 3 ~ "middle class",
...
final_df <- final_df%>%
  mutate(`Self positioning in political scale` = case_when(
    ...
    `Self positioning in political scale` == 4 ~ "Center",
    `Self positioning in political scale` == 5 ~ "Center",
    `Self positioning in political scale` == 6 ~ "Center ",
...
```{r}
# Fit a probit model
probit_model <- glm(cheat ~country+sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+ Year, data =final_df , family = binomial(link = "probit"))
```
```{r}
...
```
```{r}
final_dftwo=final_dftwo%>%mutate(cheat=as.numeric(cheat))
```
...
```
```{r}
final_dftwo=final_dftwo%>%mutate(truecheat=ifelse(cheat==1,1,0))
```
```{r}
average_by_country_truecheat  <- aggregate(truecheat ~ country, data = final_dftwo, FUN = mean)
```
...
# Assuming 'data_vector' is your data
data_vector_compressed <- (average_by_country_truecheat$truecheat)  # Replace with your actual data
...
```{r}
library(readxl)
income  <- read_excel("P_Data_Extract_From_World_Development_Indicators.xlsx", 
    col_types = c("text", "numeric", "numeric"))

```
```{r}
income$growth=(income$`2000 [YR2000]`-income$`1990 [YR1990]`)/income$`1990 [YR1990]`
```
```{r}
  income=income%>%mutate(country =as.factor(`Country Name`) )
income$`Country Name`=NULL
```

```{r}
# Merge data frames
merged_df <- inner_join(final_df, income, by = "country")
```
```{r}
...
```{r}
averageincome= aggregate(merged_df$`2000 [YR2000]`~ country, data =merged_df , FUN = mean)
```

```{r}
averagetruechat= aggregate(cheat ~ country, data =merged_df , FUN = mean)
```
```{r}
library(readxl)
world_regions_according_to_the_world_bank <- read_excel("C:/Users/gealy/Downloads/world-regions-according-to-the-world-bank.xlsx")
View(world_regions_according_to_the_world_bank)
```
```{r}
#world_regions_according_to_the_world_bank=world_regions_according_to_the_world_bank%>%mutate(`World Region according to the World Bank`=as.factor(`World Region according to the World Bank`))

world_regions_according_to_the_world_bank <- world_regions_according_to_the_world_bank %>%
  rename(region = `World Region according to the World Bank`)

```
```{r}
merged_df <- inner_join(merged_df, world_regions_according_to_the_world_bank, by = "country")
```
```{r}
...
library(readxl)
CPI_Archive_1995 <- read_excel("CPI-Archive-1995.xlsx", 
    col_types = c("text", "skip", "skip", 
        "numeric", "numeric"))
View(CPI_Archive_1995)
```

```{r}
CPI_Archive_1995_1=CPI_Archive_1995%>%mutate(country =as.factor(CPI_Archive_1995$country) )

```
```{r}
merged_df <- inner_join(merged_df, CPI_Archive_1995, by = "country")
```
...
```{r}
merged_df <- merged_df %>%
  mutate(incleval = ifelse(`2000 [YR2000]` > 20000, "high income",ifelse(`2000 [YR2000]` >= 1400 & `2000 [YR2000]` < 20000, "middle income",ifelse(`2000 [YR2000]` > 0 & `2000 [YR2000]` < 1400, "low income", "unknown"))))

```
```{r}
merged_df <- merged_df %>%
  mutate(inclevalnine0 = ifelse(`1990 [YR1990]` > 20000, "high income",ifelse(`1990 [YR1990]` >= 1400 & `1990 [YR1990]` < 20000, "middle income",ifelse(`1990 [YR1990]` > 0 & `1990 [YR1990]` < 1400, "low income", "unknown"))))
```
...
```{r}
merged_df=merged_df%>%mutate(cheat=ifelse(cheat==1,1,0))
```
```{r}
...
for (var in names(reference_levels)) {
  level <- reference_levels[[var]]
  merged_df[[var]] <- relevel(as.factor(merged_df[[var]]), ref = level)
}
```
```{r}
library(DescTools)
...
```{r}
original  <- glm(cheat ~country+sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+Year, data =merged_df , family = binomial(link = "probit"))


PseudoR2(original ,c("McFaddenAdj","McKelveyZavoina","Tjur"))
```

```{r}
 raw  <- glm(cheat ~sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+Year, data =merged_df , family = binomial(link = "probit"))
 PseudoR2(raw,c("McFaddenAdj","McKelveyZavoina","Tjur"))
```

```{r}


probit_region  <-glm(cheat ~region+sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+Year, data =merged_df , family = binomial(link = "probit"))
 PseudoR2(probit_region,c("McFaddenAdj","McKelveyZavoina","Tjur"))

```
```{r}

corruptionpluecon   <- glm(cheat ~sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+Year, data =merged_df , family = binomial(link = "probit"))
 PseudoR2(corruptionpluecon,c("McFaddenAdj","McKelveyZavoina","Tjur"))
```

```{r}

bothcorandregion <- glm(cheat ~region+growth+incleval+score+sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`Self positioning in political scale`+`religious denomination`+Year, data =merged_df , family = binomial(link = "probit"))
 PseudoR2(bothcorandregion,c("McFaddenAdj","McKelveyZavoina", "Tjur"))

```
```{r}
  
justregion <- glm(cheat ~region+Year, data =merged_df , family = binomial(link = "probit"))
 PseudoR2(justregion,c("McFaddenAdj","McKelveyZavoina","Tjur"))
```
```{r}
  
just_country <- glm(cheat ~country +Year, data =merged_df , family = binomial(link = "probit"))
 PseudoR2(just_country,c("McFaddenAdj","McKelveyZavoina","Tjur"))
```


```{r}
marginal_effects <- margins(original)
marginal_summary=summary(marginal_effects)
```

```{r}
originalreg  <- data.frame(
  Term = marginal_summary$factor,
  AME = marginal_summary$AME,
  SE = marginal_summary$SE,
  z = marginal_summary$z,
...
```
```{r}
marginal_effectstwo <- margins(justregion,data = merged_df)
marginal_summarytwo=summary(marginal_effectstwo)
just_region <- data.frame(
  Term = marginal_summarytwo$factor,
  AME = marginal_summarytwo$AME,
  SE = marginal_summarytwo$SE,
  z = marginal_summarytwo$z,
  p = marginal_summarytwo$p,
  lower = marginal_summarytwo$lower,
  upper = marginal_summarytwo$upper
)

```

```{r}
pR2(probit_modelcorruption)
```
```{r}
...
```{r}
cor(as.numeric(merged_df$incleval),as.numeric(merged_df$score))
cor(as.numeric(merged_df$cheat),as.numeric(merged_df$score))
cor(as.numeric(merged_df$mean),as.numeric(merged_df$cheat))
cor(as.numeric(merged_df$growth),as.numeric(merged_df$score))
```
