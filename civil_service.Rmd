...
wave1select <- wave1 %>%
  ...
  select("religious denomination", "civil services", "employment", "sex","cheat","country","educ","Socio-economic status of respondent","age")
...
wave2select <- wave2 %>%
  ...
  select("religious denomination", "civil services", "age", "employment", "sex","cheat","educ","Socio-economic status of respondent","country")
...
wave4select <- wave4 %>%
  ...
  select("religious denomination", "civil services", "employment", "sex","educ","cheat","Socio-economic status of respondent", "country","age")
...
final_df <- final_df %>%
  mutate(
    `civil services` = case_when(
      `civil services` == 1 ~ "highly confident",
      `civil services` == 2 ~ "high to mid confident",
      `civil services` == 3 ~ "mid to low confident",
      `civil services` == 4 ~ "low confident",
      `civil services` < 0 ~ NA_character_,  # Exclude negative values
      TRUE ~ NA_character_  # for values outside the specified range or NA
    )
  )
...
```{r}
# Fit a probit model
probit_model <- glm(cheat ~country+sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`civil services`+`religious denomination`+ Year, data =final_df , family = binomial(link = "probit"))
```
```{r}
#find r^2
pR2(probit_model)
