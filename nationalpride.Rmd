...
wave1select <- wave1 %>%
  ...
  select("religious denomination", "national pride", "sex","cheat","country","employment","educ","Socio-economic status of respondent","age")
...
wave2select <- wave2 %>%
  ...
  select("religious denomination", "national pride", "age", "employment", "sex","cheat","educ","Socio-economic status of respondent","country")
...
wave4select <- wave4 %>%
  ...
  select("religious denomination", "national pride", "employment", "sex","educ","cheat","Socio-economic status of respondent", "country","age")
...
wave3select <- wave3select %>%
  mutate(`Socio-economic status of respondent` = case_when(
    ...
    `Socio-economic status of respondent` == 3 ~ "middle class",
    `Socio-economic status of respondent` == 4 ~ "middle class", # Assuming 'Working class' is similar to 'Low middle class'
...
final_df$`national pride` <- as.numeric(as.character(final_df$`national pride`))

final_df <- final_df %>%
  mutate(`national pride`= case_when(
    `national pride` >= 0 & `national pride` <= 0.33 ~ "low pride",
    `national pride` > 0.33 & `national pride` <= 0.66 ~ "middle pride",
    `national pride` > 0.66 & `national pride` <= 1 ~ "high pride",
    TRUE ~ NA_character_  # for values outside the range or NA
  ))
...
```{r}
# Fit a probit model
probit_model <- glm(cheat ~country+sex+age+I(age^2)+educ+employment+ `Socio-economic status of respondent`+`national pride`+`religious denomination`+ Year, data =final_df , family = binomial(link = "probit"))
```
